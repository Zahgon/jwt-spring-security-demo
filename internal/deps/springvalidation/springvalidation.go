...
func (notNull) name() string            { return notNullName }
func (notNull) message() string         { return notNullMessage }
...
func (size) name() string { return sizeName }

func (s size) message() string {
	return sizeMessagePrefix + strconv.Itoa(s.min) + " and " + strconv.Itoa(s.max)
}

// Jakarta Bean Validation supplies these names and default messages; the
// original never spelled them out because Hibernate Validator did. They are
// contract, so they are named here rather than left as bare literals.
const (
	notNullName       = "NotNull"
	notNullMessage    = "must not be null"
	sizeName          = "Size"
	sizeMessagePrefix = "size must be between "
)
